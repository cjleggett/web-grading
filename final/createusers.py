...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for name in [chr(i) for i in range(97, 97 + 26)]:
            ...
            try:
                user = User.objects.create_user(f"{name}@{name}.{name}", f"{name}@{name}.{name}", name)
                user.save()
                self.stdout.write(self.style.SUCCESS(f"Successfully created user {name}@{name}.{name} with password {name} and email {name}@{name}.{name}"))
                created += 1
                if created == 2:
                    return
